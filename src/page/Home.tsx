...
const Home = () => {
    ...
    const handleUpload = (event: any, id: number) => {
    ...
    return (
        <div className='w-[90%] m-auto'>
            <div className='flex items-center justify-between my-10 mb-20'>
                <h1 className='text-gray-900 text-4xl font-bold'>Users List</h1>
                <Button onClick={showModalAddUser}>+ Add New User</Button>
            </div>
            <Table>
                <TableHeader>
                    <TableRow>
                        <TableHead className='font-black text-xl text-center'>Image</TableHead>
                        <TableHead className='font-black text-xl text-center'>Name</TableHead>
                        <TableHead className='font-black text-xl text-center'>Description</TableHead>
                        <TableHead className='font-black text-xl text-center'>Status</TableHead>
                        <TableHead className='font-black text-xl text-center w-[20%]'>Action</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                    {data?.map((e: IData) => {
                        return <TableRow>
                            <TableCell>
                                {e.images?.map((img: IImg) => {
                                    return <div key={img.id} className='flex items-center'>
                                        <img className='w-12 m-auto h-12 rounded-full' src={`${apiImage}/${img.imageName}`} alt="" />
                            ...
                            <TableCell className='font-bold text-center text-lg text-gray-600'>
                            ...
                            <TableCell>
                                <div className='flex items-center gap-3 justify-center'>
                                    ...
                                    <input checked={e.isCompleted} onClick={() => dispatch(CheckStatus(e.id))} type="checkbox" className='w-4 h-4' />
                                    <input onChange={(event) => handleUpload(event, e.id)} type="file" className='w-[15%] border' />
                                </div>
                            </TableCell>
                        </TableRow>
                    })}
            ...
            >
                <form onSubmit={handleSubmit(onsubmit)} className='flex flex-col gap-4'>
                    ...
                    />
                    <button type='submit' className='bg-blue-500 text-white py-1.5 w-full rounded-xl font-bold'>Save</button>
                </form>
            </Modal>
            <Modal
                title="Add Modal"
