...
import { AddNewUser, DeleteUser, EditUser, loadableAtom, searchAtom, selectAtom } from '../atom/ToDoJotai';
...
const Jotai = () => {
  ...
  const [, Deleteuser] = useAtom(DeleteUser)
  const [, AddUser] = useAtom(AddNewUser)
  const [, EditUsers] = useAtom(EditUser)
  ...
  return (
    <>
      <h1 className='text-4xl font-bold mb-5'>Users Table JOtai</h1>
      <div className='flex mt-8 mb-5 items-center justify-between'>
        <Input value={search} onChange={(e) => setSearch(e.target.value)} style={{ width: "350px" }} placeholder='Search...' />
        <Select
          value={select}
      ...
      {state == "hasData" && (
        <table className='text-center w-full mt-10 shadow-2xl rounded-4xl'>
          <thead className='bg-blue-800 font-bold text-white'>
            <tr className=''>
              ...
              return <tr className='border-b border-b-gray-100 hover:bg-blue-100' key={e.id}>
                ...
                <td>
                  <div className='flex items-center gap-5 justify-center'>
                    ...
                    <input checked={e.status} onChange={() => EditUsers({ ...e, status: !e.status })} type="checkbox" className='w-3.5 h-3.5' />
                  </div>
                </td>
              </tr>
            })}
